...
class SmartPaySecurity:
    ...
    def encryptionkey(self):
        return Fernet(self.data.get("apikey"))
    ...
    def veryfypassword(self) -> bool:
        return self.decrypt(self.data.get("stored_pass")) == self.data["input_pass"]
    ...
    def decrypt(self, stored_pass=None) -> str:
        return self.encryptionkey.decrypt(stored_pass if stored_pass else self.data["message"]).decode()